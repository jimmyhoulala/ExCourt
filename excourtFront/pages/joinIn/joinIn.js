Page({
  ...
  getUploaderId() {
    // 向后端请求获取 uploader_id
    wx.request({
      url: 'http://123.60.86.239:8000/teamup/get_uploader_id',  // 替换成实际的获取 uploader_id 的 API 地址
      method: 'POST',
      data: {
  ...
  respondToTeamup() {
    ...
    // 调用后端接口响应换场请求
    wx.request({
      url: 'http://123.60.86.239:8000/teamup/record', // 后端 API 地址
      method: 'POST',
      data: {
        Student_id: my_id,  // 响应者学号
  ...
  gotoChat(){
    wx.request({
      ...
      data: {
          ...
          Message_sent: '球友你好！' + '我想要加入你的' + this.data.courtid + '谢谢！',
      },
      success: (sendRes) => {
